...
```{r, results='asis'}
print(xtable(dtDaily[, list(meanBeforeImputing = mean(sumSteps), medianBeforeImputing = median(sumSteps))]), type="html", include.rownames=FALSE)
```
...
```{r}
dtDaily <- dt[, list(sumSteps = sum(steps, na.rm=TRUE), isImputed = sum(steps_imp) > 0), date]
dtDaily
```
...
ggplot(dtDaily, aes(x=date, y=sumSteps, fill=isImputed)) +
...
```{r, results='asis'}
print(xtable(dtDaily[, list(meanAfterImputing = mean(sumSteps), medianAfterImputing = median(sumSteps))]), type="html", include.rownames=FALSE)
```
