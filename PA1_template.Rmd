...
```{r}
packages <- c("data.table", "ggplot2", "xtable", "mice")
...
Use the [mice](http://cran.r-project.org/web/packages/mice/index.html) package to impute missing values of the `steps` variable.
Mice generates Multivariate Imputations by Chained Equations.
Use the predicted values from this model to impute missing values; call this variable `stepsImputed`.

```{r}
imputed <- mice(dt[, list(steps, dateNumeric = as.numeric(date), interval)])
```
