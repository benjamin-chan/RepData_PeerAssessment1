...
```{r}
dtDaily <- dt[, list(sumSteps = sum(steps, na.rm=TRUE), sumStepsImputed = sum(stepsImputed)), date]
dtDaily
```
...
ggplot(dtDaily, aes(x=date, y=sumStepsImputed)) +
	geom_histogram(stat="identity")
```
...
```{r, results='asis'}
print(xtable(dtDaily[, list(mean = mean(sumStepsImputed), median = median(sumStepsImputed))]), type="html", include.rownames=FALSE)
```
